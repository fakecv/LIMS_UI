      ...
      <el-form :model="customerForm" label-width="100px" label-position="left" size="mini">
        ...
        <el-row :gutter="20">
          <el-form-item label="客户电话">
            <el-input name="mobileNumber" v-model="customerForm.mobileNumber" autoComplete="mobileNumber"></el-input>
          </el-form-item>
        </el-row>
        <el-row :gutter="20">
          <el-form-item label="客户传真">
            <el-input name="fax" v-model="customerForm.fax" autoComplete="fax"></el-input>
          </el-form-item>
        </el-row>
        <el-row :gutter="20">
          <el-form-item label="客户邮箱">
            <el-input name="email" v-model="customerForm.email" autoComplete="email"></el-input>
          </el-form-item>
        </el-row>
        <el-row :gutter="20">
          <el-form-item label="客户地址">
            <el-input type="textarea" name="address" v-model="customerForm.address" autoComplete="address"></el-input>
          </el-form-item>
        </el-row>
      </el-form>
    </el-container>
...
export default {
  ...
  methods: {
    ...
    saveToDB () {
      let vm = this
      this.$ajax.post('/api/customer', this.customerForm)
        .then(function (res) {
          vm.$message('已经成功保存到数据库!')
          vm.$emit('updateCustomerForm', res.data)
    ...
    confirmDelete () {
      let vm = this
      if (this.customerForm.id && this.customerForm.id !== '') {
    ...
    delete () {
      let vm = this
      this.$ajax.get('/api/customer/delete/' + this.customerForm.id)
        .then(function (res) {
          vm.$message('已经成功删除！')
          vm.$emit('deleteCustomer')
        }).catch(function (error) {
          vm.$message(error.response.data.message)
        })
    }
