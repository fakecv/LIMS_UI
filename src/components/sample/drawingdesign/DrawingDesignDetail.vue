      ...
      <el-form :model="drawingDesignForm" label-width="100px" label-position="left" size="mini">
        ...
        <el-row :gutter="20">
          <el-form-item label="部门描述">
            <el-input type="textarea" name="drawingDesignDescription" v-model="drawingDesignForm.drawingDesignDescription"></el-input>
          </el-form-item>
        </el-row>
      </el-form>
    </el-container>
...
export default {
  ...
  methods: {
    ...
    saveToDB () {
      let vm = this
      this.$ajax.post('/api/sample/drawingDesign', this.drawingDesignForm)
        .then(function (res) {
    ...
    delete () {
      let vm = this
      this.$ajax.get('/api/sample/drawingDesign/delete/' + this.drawingDesignForm.id)
        .then(function (res) {
          vm.$message('已经成功删除！')
        }).catch(function (error) {
          console.log('DrawingDesignDetail delete ' + error)
          vm.$message('Something wrong happen!')
        })
    }
  }
