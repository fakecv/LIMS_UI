...
<script>
export default {
  name: 'availableEquipmentDetail',
  props: ['availableEquipmentRequestForm', 'staticOptions'],
  data () {
    return {
      actions: [
  ...
  methods: {
    ...
    saveToDB () {
      let vm = this
      this.$ajax.post('/api/sample/availableEquipment', this.availableEquipmentRequestForm)
        .then(function (res) {
          vm.$message('已经成功保存到数据库!')
          vm.$emit('updateAvailableEquipmentForm', res.data)
    ...
    confirmDelete () {
      let vm = this
      if (this.availableEquipmentForm.id && this.availableEquipmentRequestForm.id !== '') {
    ...
    delete () {
      let vm = this
      this.$ajax.get('/api/sample/availableEquipment/delete/' + this.availableEquipmentRequestForm.id)
        .then(function (res) {
          vm.$message('已经成功删除！')
          vm.$emit('deleteAvailableEquipmentForm')
