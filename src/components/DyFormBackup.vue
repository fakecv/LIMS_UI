  ...
  <el-container>
    <el-form :inline="true" label-width="80px" size="mini" :ref="formName">
      <el-form-item v-for="(item, index) in formData" :key='index' :label="item.name" width="item.width">
        ...
        <el-col v-if="item.type === 'col'" :span="item.val"></el-col>
        <el-select v-if="item.type === 'select'" v-model="item.val" :placeholder="item.val">
          <el-option v-for="(selectOption,selectOptionIndex) in item.child" :key="selectOptionIndex" v-if="selectOption.type === 'selectOption'" :label="selectOption.name"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item>
