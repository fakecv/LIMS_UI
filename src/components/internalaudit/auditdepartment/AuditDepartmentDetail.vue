<template>
  <el-container>
    ...
    <el-container style="padding: 10px">
      <el-form :model="auditDepartmentForm" label-width="100px" label-position="left" size="mini">
        <el-row :gutter="20">
          ...
          <el-col :lg="columnSize.lg" :md="columnSize.md" :xl="columnSize.xl" :xs="columnSize.xs" :sm="columnSize.sm">
            <el-form-item label="被审核岗位描述">
              <el-input name="auditDepartmentDescription" v-model="auditDepartmentForm.auditDepartmentDescription" autoComplete="auditDepartmentDescription"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    saveToDB () {
      let vm = this
      this.$ajax.post('/api/auditdepartment/auditDepartment', this.auditDepartmentForm)
        .then(function (res) {
    ...
    delete () {
      let vm = this
      this.$ajax.get('/api/auditdepartment/auditDepartment/delete/' + this.auditDepartmentForm.id)
        .then(function (res) {
          vm.$message('已经成功删除！')
