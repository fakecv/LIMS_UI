...
<script>
export default {
  name: 'wangxingangDetail',
  props: ['wangxingangForm', 'staticOptions'],
  data () {
    return {
      actions: [
  ...
  methods: {
    ...
    saveToDB () {
      let vm = this
      this.$ajax.post('/api/equipment/wangxingang', this.wangxingangForm)
        .then(function (res) {
          vm.$message('已经成功保存到数据库!')
          vm.$emit('updateWangxingangForm', res.data)
    ...
    confirmDelete () {
      let vm = this
      if (this.wangxingangForm.id && this.wangxingangForm.id !== '') {
    ...
    delete () {
      let vm = this
      this.$ajax.get('/api/equipment/wangxingang/delete/' + this.wangxingangForm.id)
        .then(function (res) {
          vm.$message('已经成功删除！')
          vm.$emit('deleteWangxingangForm')
