...
<script>
export default {
  name: 'generalApplicanceRequestDetail',
  props: ['generalApplicanceRequestForm', 'staticOptions'],
  data () {
    return {
      actions: [
  ...
  methods: {
    ...
    saveToDB () {
      let vm = this
      this.$ajax.post('/api/equipment/generalApplicanceRequest', this.generalApplicanceRequestForm)
        .then(function (res) {
          vm.$message('已经成功保存到数据库!')
          vm.$emit('updateGeneralApplicanceRequestForm', res.data)
    ...
    confirmDelete () {
      let vm = this
      if (this.generalApplicanceRequestForm.id && this.generalApplicanceRequestForm.id !== '') {
    ...
    delete () {
      let vm = this
      this.$ajax.get('/api/equipment/generalApplicanceRequest/delete/' + this.generalApplicanceRequestForm.id)
        .then(function (res) {
          vm.$message('已经成功删除！')
          vm.$emit('deleteGeneralApplicanceRequestForm')
